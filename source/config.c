...
int write_config(const char *file) {
  ...
  //
  // The file is rewritten on every launch: your values are preserved, but
  ...
  fprintf(f, "\n# Face button layout.\n"
             ...
             "#       (left), A = Circle (right), B = Cross (bottom).\n"
  ...
  fprintf(f, "\n# Button mapping. One entry per physical Switch button; the value is an\n"
             ...
             "#   CAM_CENTER           - centre the camera behind the player\n"
             "#   NONE                 - button does nothing\n");
  CONFIG_BUTTON_LINE(key_a);
  CONFIG_BUTTON_LINE(key_b);
  CONFIG_BUTTON_LINE(key_x);
  CONFIG_BUTTON_LINE(key_y);
